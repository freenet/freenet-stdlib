// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers';

import { ContractKey, ContractKeyT } from '../common/contract-key.js';


export class SubscribeResponse implements flatbuffers.IUnpackableObject<SubscribeResponseT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):SubscribeResponse {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsSubscribeResponse(bb:flatbuffers.ByteBuffer, obj?:SubscribeResponse):SubscribeResponse {
  return (obj || new SubscribeResponse()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsSubscribeResponse(bb:flatbuffers.ByteBuffer, obj?:SubscribeResponse):SubscribeResponse {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new SubscribeResponse()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

key(obj?:ContractKey):ContractKey|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? (obj || new ContractKey()).__init(this.bb!.__indirect(this.bb_pos + offset), this.bb!) : null;
}

subscribed():boolean {
  const offset = this.bb!.__offset(this.bb_pos, 6);
  return offset ? !!this.bb!.readInt8(this.bb_pos + offset) : false;
}

static startSubscribeResponse(builder:flatbuffers.Builder) {
  builder.startObject(2);
}

static addKey(builder:flatbuffers.Builder, keyOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, keyOffset, 0);
}

static addSubscribed(builder:flatbuffers.Builder, subscribed:boolean) {
  builder.addFieldInt8(1, +subscribed, +false);
}

static endSubscribeResponse(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  builder.requiredField(offset, 4) // key
  return offset;
}

static createSubscribeResponse(builder:flatbuffers.Builder, keyOffset:flatbuffers.Offset, subscribed:boolean):flatbuffers.Offset {
  SubscribeResponse.startSubscribeResponse(builder);
  SubscribeResponse.addKey(builder, keyOffset);
  SubscribeResponse.addSubscribed(builder, subscribed);
  return SubscribeResponse.endSubscribeResponse(builder);
}

unpack(): SubscribeResponseT {
  return new SubscribeResponseT(
    (this.key() !== null ? this.key()!.unpack() : null),
    this.subscribed()
  );
}


unpackTo(_o: SubscribeResponseT): void {
  _o.key = (this.key() !== null ? this.key()!.unpack() : null);
  _o.subscribed = this.subscribed();
}
}

export class SubscribeResponseT implements flatbuffers.IGeneratedObject {
constructor(
  public key: ContractKeyT|null = null,
  public subscribed: boolean = false
){}


pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  const key = (this.key !== null ? this.key!.pack(builder) : 0);

  return SubscribeResponse.createSubscribeResponse(builder,
    key,
    this.subscribed
  );
}
}
